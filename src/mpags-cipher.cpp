...
int main(int argc, char* argv[])
{
    ...
    const bool cmdLineStatus{
        processCommandLine(cmdLineArgs, program_settings)};
    ...
    if (!program_settings.inputFile.empty()) {
        ...
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << program_settings.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    if (!program_settings.cipherKey.empty()) {
        ...
        for (const auto& elem : program_settings.cipherKey) {
            if (!std::isdigit(elem)) {
                std::cerr
                    << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
                    << "        the supplied key (" << program_settings.cipherKey
                    << ") could not be successfully converted" << std::endl;
                return 1;
            }
        }
        caesarKey = std::stoul(program_settings.cipherKey);
    }

    // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
    std::string outputText{runCaesarCipher(inputText, caesarKey, program_settings.encrypt)};
    ...
    if (!program_settings.outputFile.empty()) {
        ...
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << program_settings.outputFile << "'" << std::endl;
            return 1;
        }
